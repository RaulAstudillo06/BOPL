...
from ..util.linalg import pdinv, dpotrs, tdot, dtrtrs, jitchol
...
class GP(Model):
    ...
    def partial_precomputation_for_covariance_conditioned_on_next_point(self, next_point):
        ...
        kern = self.kern
        self.X_next = np.append(self.X, next_point, axis=0)
        K_next = kern.K(self.X_next)
        noise_var = self.likelihood.variance
        diag.add(K_next, noise_var+1e-8)
        #K_aux += np.eye(K_aux.shape[0])*noise_var
        woodbury_chol = pdinv(K_next)[1]
        self.partial_precomp_covariance_conditioned_on_next_point = woodbury_chol
    ...
    def chol_posterior_covariance_conditioned_on_next_point(self, X):
        ...
        Kx = kern.K(self.X_next, X)
        Kxx = kern.K(X)
        tmp = dtrtrs(self.partial_precomp_covariance_conditioned_on_next_point, Kx)[0]
        cov = Kxx - tdot(tmp.T)
        #print('test begin')
        #print(cov)
        #print('test end')
    ...
    def posterior_covariance_gradient(self, X, X2, kern=None):
        ...
        if False:
            print('test')
            print(pos_cov_grad)
            print(pos_cov_grad.shape)
            for n in range(X2.shape[0]):
                x2 = np.atleast_2d(X2[n,:])
                factor = np.matmul(kern.K(x2, self._predictive_variable), self.posterior.woodbury_inv)
                print(kern.gradients_X(None, X, x2) - np.matmul(factor,kern.gradients_X(None, X, self._predictive_variable)))
        return pos_cov_grad
