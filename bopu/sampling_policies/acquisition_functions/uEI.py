...
class uEI(AcquisitionBase):
    ...
    def _marginal_acq(self, X, utility_params_samples):
        ...
        for h in range(self.n_hyps_samples):
            ...
            for l in range(L):
                max_valX_evaluated = np.max(self.utility.eval_func(fX_evaluated, utility_params_samples[l]))
                for W in self.W_samples:
                    for i in range(X.shape[0]):
                        valx = self.utility.eval_func(muX[:, i] + sigmaX[:, i]*W, utility_params_samples[l])
    ...
    def _parallel_acq_helper(self, x):
        ...
        for l in range(L):
            max_valX_evaluated = np.max(self.utility.eval_func(fX_evaluated, utility_params_samples[l]))
            for W in self.W_samples:
                valx = self.utility.eval_func(mux + sigmax * W, utility_params_samples[l])
